...
class LogDataset(Dataset):
    ...
    def collate_fn(self, batch, percentile=100, dynamical_pad=True):
        ...
        output["bert_input"] = torch.from_numpy(np.array(output["bert_input"]))
        output["bert_label"] = torch.from_numpy(np.array(output["bert_label"]))
        output["time_input"] = torch.from_numpy(np.array(output["time_input"]))
        output["time_label"] = torch.from_numpy(np.array(output["time_label"]))
